...
const LOCAL_ANIMAL_DISTANCE_METERS = 3
...
function buildLocalTestAnimals(animals: Animal[], location: LocationData): Animal[] {
  return animals.slice(0, 5).map((animal, index) => {
    const point = offsetCoordinate(location.latitude, location.longitude, LOCAL_ANIMAL_DISTANCE_METERS, LOCAL_ANIMAL_BEARINGS[index])
    return { ...animal, latitude: point.latitude, longitude: point.longitude, altitude: (location.altitude ?? animal.altitude) + (index === 0 ? 0 : 1) }
  })
}
...
export function CameraPage({ onNavigate }: CameraPageProps) {
  ...
  const start3DAr = async () => {
    ...
    try {
      // Use the same localized animals that are visible in the camera fallback.
      // This keeps the real 3D AR mode populated even when the database animals
      // are far from the user's current GPS position.
      const xrAnimals = animalPositions.length > 0 ? animalPositions.map((item) => item.animal) : activeAnimals
      const session = await startWebXRAnimalSession(xrAnimals, { ...filteredLocation, heading }, () => { xrSessionRef.current = null; setXrActive(false) })
      if (!session) { setXrUnavailable(true); return }
      session.addEventListener('select', selectBestCenteredAnimal as EventListener)
      session.addEventListener('selectstart', selectBestCenteredAnimal as EventListener)
