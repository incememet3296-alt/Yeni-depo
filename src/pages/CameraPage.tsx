...
export function CameraPage({ onNavigate }: CameraPageProps) {
  ...
  const [selected, setSelected] = useState<Animal | null>(null); const [viewport, setViewport] = useState({ width: window.innerWidth, height: window.innerHeight }); const [xrStarting, setXrStarting] = useState(false); const [xrActive, setXrActive] = useState(false); const [xrUnavailable, setXrUnavailable] = useState(false); const xrSessionRef = useRef<WebXRSession>(null); const containerRef = useRef<HTMLDivElement>(null); const headingRef = useRef<number | null>(null); const locationRef = useRef<LocationData | null>(null);
  ...
  const start3DAr = async () => { if (!filteredLocation || heading == null || xrStarting || xrActive) return; setXrStarting(true); setXrUnavailable(false); try { const session = await startWebXRAnimalSession(activeAnimals, { ...filteredLocation, heading }, () => { xrSessionRef.current = null; setXrActive(false) }); if (!session) { setXrUnavailable(true); return } xrSessionRef.current = session; setXrActive(true) } catch { setXrUnavailable(true) } finally { setXrStarting(false) } }
