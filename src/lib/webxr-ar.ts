...
interface XRPoseLike { transform: { inverse: { matrix: Float32Array } }; views: XRViewLike[] }
interface XRViewLike { projectionMatrix: Float32Array; transform: { matrix: Float32Array } }
  ...
  const render = (_time: number, frame: XRFrameLike) => {
    ...
    for (const view of pose.views) {
      ...
      for (const animal of animals) {
        ...
        const transform = modelMatrix(world.east, world.up + 1.2, -world.north, Math.max(0.25, Math.min(2.5, 8 / Math.max(3, world.distance))))
