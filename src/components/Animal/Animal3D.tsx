...
// WebGL expects matrices in column-major order. The previous implementation
// multiplied them as row-major matrices, which could leave the transparent
// canvas on screen while the 3D geometry itself was clipped/invisible.
    ...
    try {
      ...
      const render = (now: number) => {
        ...
        for (let i = 0; i < parts.length; i += 1) {
          ...
          gl.uniformMatrix4fv(modelLocation, false, new Float32Array(animated))
          gl.uniformMatrix4fv(mvpLocation, false, new Float32Array(multiply(base, animated)))
      ...
      console.error('Animal3D WebGL başlatılamadı:', error)
    }
  }, [rarity, size])
